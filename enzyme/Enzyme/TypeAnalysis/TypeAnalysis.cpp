...
#define CONSIDER2(fn, ...)                                                     \
  if (funcName == #fn) {                                                       \
    ...
    if (funcName == "__kmpc_fork_call") {
      ...
      if (fn) {
          if (call.getNumArgOperands() - 3 != fn->getFunctionType()->getNumParams() - 2)
            return;
          ...
          for (auto &arg : fn->args()) {
            if (argnum <= 1) {
              ...
              std::set<int64_t> bounded;
                for (auto v : fntypeinfo.knownIntegralValues(call.getArgOperand(argnum - 2 + 3),
                                                             *DT, intseen)) {
    ...
    if (auto AI = dyn_cast<AllocaInst>(LI->getPointerOperand())) {
      ...
      for (auto u : AI->users()) {
        if (auto SIu = dyn_cast<StoreInst>(u)) {
          ...
          if (!cast<Instruction>(u)->mayReadOrWriteMemory() && cast<Instruction>(u)->use_empty())
              continue;
